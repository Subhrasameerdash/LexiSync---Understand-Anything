...
# Suppress visually noisy transformers warnings
...
def initialize_pipeline():
    ...
    try:
        ...
        with st.spinner("Initializing LexiSync pipeline..."):
            ...
            rag_generator = RAGGenerator(
                ...
                vector_db=vector_db,
                hf_api_token=hf_token  # Changed: was dashscope_api_key
            )
            ...
            # Changed: PodcastScriptGenerator now takes hf_api_token
            podcast_script_generator = PodcastScriptGenerator(hf_api_token=hf_token) if hf_token else None
            
            try:
                podcast_tts_generator = PodcastTTSGenerator() if hf_token else None
...
def generate_podcast(selected_source: str, podcast_style: str, podcast_length: str):
    if not st.session_state.pipeline or not st.session_state.pipeline['podcast_script_generator']:
        st.error("Podcast generation not available. Please check your HUGGINGFACEHUB_API_TOKEN.")
        return
...
def main():
    ...
    <div style="display: flex; align-items: center; margin-bottom: 30px;">
        <h1 style="color: #ffffff; margin: 0;">🧠 LexiSync: Understand Anything</h1>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #a0aec0; font-size: 12px;">
        LexiSync can be inaccurate; please double check its responses.
    </div>
    """, unsafe_allow_html=True)
