...
class PodcastScriptGenerator:
    def __init__(
        ...
        # Legacy kwarg aliases kept so existing callers don't break
        ...
        resolved_token = (
            hf_api_token
            or dashscope_api_key          # graceful fallback (not actually a HF token)
            or openai_api_key
            or os.getenv("HUGGINGFACEHUB_API_TOKEN")
        )

        # Build the InferenceClient once — reused for all generate calls
        ...
        logger.info(f"Podcast script generator initialised with HF model: {model_name}")

    # ------------------------------------------------------------------
    # Private: thin wrapper around HF chat_completion
    # ------------------------------------------------------------------
    def _call_hf(self, prompt: str, max_tokens: int = 4000) -> str:
        """
        Generate text via Qwen2.5-7B-Instruct on HF Serverless Inference.

        Uses the OpenAI-compatible chat_completion interface.
    ...
    def generate_script_from_document(
        ...
        script_data = self._generate_conversation_script(
            document_content, podcast_style, target_duration
        )
    ...
    def generate_script_from_text(
        ...
        script_data = self._generate_conversation_script(
            text_content, podcast_style, target_duration
        )
    ...
    def generate_script_from_website(
        ...
        script_data = self._generate_conversation_script(
            website_content, podcast_style, target_duration
        )
    ...
    def _generate_conversation_script(
        ...
        style_prompts = {
            "conversational": (
                "Create a natural, friendly conversation between two hosts discussing the document. "
                "They should build on each other's points and occasionally ask clarifying questions."
            ),
            "educational": (
                "Create an educational discussion where one speaker explains concepts and the other "
                "asks thoughtful questions to help clarify complex topics for listeners."
            ),
            "interview": (
                "Create an interview format where Speaker 1 acts as the interviewer asking questions "
                "and Speaker 2 provides detailed explanations from the document."
            ),
            "debate": (
                "Create a thoughtful discussion where speakers present different perspectives on the "
                "topics, maintaining respect while exploring various viewpoints."
            ),
        }
...
3. Use engaging, conversational language that is easy to understand.
4. Include brief introductions at the start and a wrap-up at the end.
5. Break down complex concepts into digestible explanations.
6. Maintain professional grammar and punctuation throughout.
7. Make it engaging for listeners who have not read the document.

RESPONSE FORMAT:
Respond with ONLY a valid JSON object containing a 'script' array. Each array element must be an object with either 'Speaker 1' or 'Speaker 2' as the key and their dialogue as the value. Do NOT include any text outside the JSON.

Example format:
{{
  "script": [
    {{"Speaker 1": "Welcome everyone to our podcast! Today we are diving into some fascinating insights from this document..."}},
    {{"Speaker 2": "Thanks for having me! I am really excited to discuss this topic. The first thing that caught my attention was..."}}
  ]
}}
...
Generate an engaging {target_duration} podcast script now:"""
        ...
        try:
            ...
            if response_clean.startswith("```json"):
                response_clean = response_clean[7:].rstrip("` \n")
            elif response_clean.startswith("```"):
                response_clean = response_clean[3:].rstrip("` \n")
            ...
            try:
                ...
                raise ValueError(
                    f"Could not parse LLM response as valid JSON. Raw response:\n{response}"
                )
    ...
    def _validate_and_clean_script(
...
if __name__ == "__main__":
    ...
    generator = PodcastScriptGenerator(hf_api_token=hf_token)
    ...
    try:
        ...
        has revolutionised fields like computer vision, natural language processing, and speech recognition.
        ...
        and job displacement — remain important challenges that need to be addressed as the technology evolves.
        """
