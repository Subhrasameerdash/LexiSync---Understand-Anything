...
class NotebookMemoryLayer:
    def __init__(
        ...
        # Changed: Zep client is initialised, removing all CrewAI wrappers
    ...
    def save_conversation_turn(
        ...
        try:
            ...
            # Save user message and assistant response using native zep-cloud SDK
            self.zep_client.thread.add_messages(
                thread_id=self.session_id,
                messages=[
                    Message(role="user", content=rag_result.query, metadata=user_meta),
                    Message(role="assistant", content=rag_result.response, metadata=assistant_meta)
                ]
            )
    ...
    def _save_source_context(self, sources_used: List[Dict[str, Any]]):
        ...
        self.zep_client.thread.add_messages(
            ...
            messages=[
                Message(
                    role="system",
                    content=f"Document sources referenced: {source_context}",
                    metadata={
                        "type": "source_context",
            ...
            if getattr(results, "episodes", None):
                for ep in results.episodes:
                    memory_info = {
                        ...
                        "session_id": ep.session_id if getattr(ep, 'session_id', None) else None,
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        try:
            ...
            if not getattr(messages, "messages", None):
                return {"message_count": 0, "summary": "No messages in session"}
            
            user_messages = [m for m in messages.messages if getattr(m, "role", "") == "user"]
            assistant_messages = [m for m in messages.messages if getattr(m, "role", "") == "assistant"]
            ...
            summary = {
                ...
                "last_interaction": messages.messages[-1].created_at if messages.messages else None
            }
...
if __name__ == "__main__":
    ...
    if not os.getenv("ZEP_API_KEY"):
        print("Please set ZEP_API_KEY environment variable to test")
        exit(0)
